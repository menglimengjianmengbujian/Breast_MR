...
class CustomResNet50(nn.Module):
    def __init__(self,  in_channels=3,num_classes=2, chunk=4,csv_shape = 107,CSV=True):
    ...
    #可以设置是否冻结权重，默认为不冻结权重，如果为False，则所有权重都会被更新
        ...
        self.fc = nn.Linear(2048 * self.chunk , num_classes)
        self.fc1 = nn.Linear(csv_shape , 2048)
    ...
    def forward(self, data_DCE,data_T2,data_T1,csv):
        ...
        csv = csv
        if self.CSV==False:
            csv1 = torch.ones_like(data_DCE)
            x = torch.cat((data_DCE, data_T2,data_T1,csv1), dim=1)
            output = self.fc(x)
        else:
            csv2 = self.fc1(csv)
            # 在全连接层之前进行拼接
            x = torch.cat((data_DCE,data_T2,data_T1,csv2), dim=1)
            # 全连接层输出
            output = self.fc(x)
...
if __name__ == '__main__':
    # 创建一个实例，并指定类别数
    net = CustomResNet50( in_channels=3,num_classes=2,chunk=4,csv_shape = 107,CSV=True)
    # 打印每一层的名称以及其参数是否可训练
