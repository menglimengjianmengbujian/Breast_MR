...
class CESM(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # sample = {'data_DCE': data_DCE, 'data_T2': data_T2,'data_T1': data_T1,'csv': csv,
        #           'label': label} #三输入融合
        #
        # sample = {'data_DCE': data_DCE, 'data_T2': data_DCE, 'data_T1': data_DCE,'csv': csv,
        #           'label': label} # 都是DCE
        
        # sample = {'data_DCE': data_T2, 'data_T2': data_T2,'data_T1': data_T2,'csv': csv,
        #           'label': label} #都是T2

        sample = {'data_DCE': data_T1, 'data_T2': data_T1,'data_T1': data_T1,'csv': csv,
                  'label': label} #都是T1
...
if __name__ == "__main__":
    ...
    dataset = CESM(base_dir=r"C:\Users\Administrator\Desktop\Breast\data\train", transform=transform)
    dataloader = DataLoader(dataset, batch_size=4, shuffle=True)

    for i, sample in enumerate(dataloader):
        print(f"批次 {i + 1}")
        print(f"数据形状: {sample['data_DCE'].shape}")
