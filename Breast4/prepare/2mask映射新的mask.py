...
nii文件不但有图像的值信息还有位置等其他信息，所以将mask插值为T2图像的大小不能用于特征提取；而是在dcm文件转化为nii过程中今替换里面的图像的值，不改变其他信息。
...
def dcm2nii(dcmdir_path, mask_path, savenii_path):
    ...
    label_array = sitk.GetArrayFromImage(label_nii).astype(np.float32)
    label_array = F.interpolate(torch.tensor(label_array, dtype=torch.float32).unsqueeze(0), size=tuple(shape),
...
dcms_root_dir_path = r"C:\Users\Administrator\Desktop\Breast\ki67原始数据\高表达\T2"
mask_dir_path = r"C:\Users\Administrator\Desktop\Breast\ROI\High\DCE"
savenii_dir = r"C:\Users\Administrator\Desktop\Breast\ROI\High\T2"
